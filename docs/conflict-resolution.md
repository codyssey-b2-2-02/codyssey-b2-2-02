...
- 선택한 해결 전략: choose one (같은 예시 줄의 계산 결과값이라 양쪽 유지 불가능)
...
- main 브랜치의 최신 내역을 반영하기 위해 feature/rebase-test 브랜치에서 git rebase 수행 (베이스 커밋: cbb8c9699bcf701c1e3c7a7683b125310d292a84)
- 동일한 파일(src/add_operation.py)의 같은 줄이 리베이스 적용 과정에서 자명한(hunk) 충돌을 일으켜 작업이 일시 정지됨
...
- 에디터로 src/add_operation.py 파일을 열어 `<<<<<<<`, `=======`, `>>>>>>>` 충돌 마커 줄을 완전히 제거하고 최종 남길 코드로 정돈함
- `git add src/add_operation.py` 명령어를 실행하여 충돌 해결 상태를 스테이징 영역(Index)에 등록함
...
- Rebase 진행 중 HEAD의 의미: Rebase 충돌 시 `<<<<<<< HEAD`는 내 작업 브랜치가 아니라, 리베이스의 베이스(Base)가 되는 대상 브랜치(main)의 커밋 상태를 가리킴을 이해함
